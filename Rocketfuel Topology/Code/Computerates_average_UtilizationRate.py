...
def AverageUtilization_metric(seedvalue,use_sol):
  trace_file="results/pitUtilization_use_"+str(use_sol)+"_rocket_seed_"+str(seedvalue)+"_.txt"
  res_file="work/average/AverageUtilizationRate_use_"+str(use_sol)+"_rocket_seed_"+str(seedvalue)+"_.txt"
  f1=open(res_file,'w')
  i=1
...
def AverageUtilizationRatePerTime(time,use_sol):
	...
	while(j<len(seed_list)):
		seedvalue=seed_list[j]
		file_name="work/average/AverageUtilizationRate_use_"+str(use_sol)+"_rocket_seed_"+str(seedvalue)+"_.txt"
		with open(file_name,'r') as f:
			for line in f:
				fields=line.split("\t")
